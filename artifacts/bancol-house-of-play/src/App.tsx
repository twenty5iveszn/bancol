...
function Mark({ inverse = false }: { inverse?: boolean }) {
  return (
    <span className={`flex items-center gap-3 ${inverse ? 'text-[hsl(var(--background))]' : 'text-[hsl(var(--foreground))]'}`}>
      ...
      <span className="font-mono-label text-[11px] font-medium tracking-[.22em]">BANCOL</span>
    </span>
  );
}
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-40 transition-all duration-300 ${scrolled ? 'border-b border-[hsl(var(--border)/.8)] bg-[hsl(var(--background)/.92)] backdrop-blur-md' : 'bg-transparent'}`}>
      <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-5 md:px-10">
        <a href="#top" aria-label="BANCOL home"><Mark /></a>
...
function Hero({ onRead }: { onRead: () => void }) {
  return (
    <section id="top" className="relative flex min-h-[92dvh] items-end overflow-hidden bg-[hsl(var(--secondary))] text-[hsl(var(--background))]">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_72%_30%,hsl(var(--primary)/.32),transparent_36%),linear-gradient(110deg,hsl(var(--secondary))_34%,transparent_77%)]" />
      <img src="/kintu-nambi-hero.png" alt="Two figures beneath a great tree at dawn" className="absolute inset-0 h-full w-full object-cover opacity-65 mix-blend-screen" />
      ...
      <div className="relative mx-auto grid w-full max-w-[1440px] items-end gap-12 px-5 pb-14 pt-36 md:grid-cols-[1.15fr_.85fr] md:px-10 md:pb-20">
        <div className="reveal max-w-4xl">
          <p className="font-mono-label mb-7 text-[10px] uppercase tracking-[.24em] text-[hsl(var(--accent))]">A legend from Buganda · Central Uganda</p>
          <h1 className="font-display text-[clamp(4.8rem,13vw,12.5rem)] leading-[.76] tracking-[-.045em]">Kintu<br /><span className="ml-[.16em] italic text-[hsl(var(--accent))]">&amp; Nambi</span></h1>
          <div className="mt-10 flex flex-wrap items-center gap-4">
            <button onClick={onRead} className="group flex items-center gap-3 rounded-full bg-[hsl(var(--accent))] px-5 py-3 font-mono-label text-[10px] uppercase tracking-[.12em] text-[hsl(var(--secondary))] transition-transform hover:scale-[1.03]">Read the legend <ArrowDown size={14} className="transition-transform group-hover:translate-y-1" /></button>
            <a href="#listen" className="flex items-center gap-3 rounded-full border border-[hsl(var(--background)/.4)] px-5 py-3 font-mono-label text-[10px] uppercase tracking-[.12em] transition-colors hover:border-[hsl(var(--accent))] hover:text-[hsl(var(--accent))]"><Headphones size={14} /> Hear the story</a>
          </div>
        </div>
      ...
      <Spiral className="absolute -bottom-12 right-[-10px] h-44 w-44 text-[hsl(var(--accent)/.55)] md:h-64 md:w-64" />
    </section>
  );
}
    ...
    <section className="bg-[hsl(var(--background))] px-5 py-24 md:px-10 md:py-36">
      <div className="mx-auto grid max-w-[1180px] gap-12 md:grid-cols-[.36fr_1fr]">
        ...
        <div>
          ...
          <p className="mt-10 max-w-2xl text-lg leading-relaxed text-[hsl(var(--muted-foreground))] md:text-xl">BANCOL is a living home for African stories, games, songs and cultural experiences. We begin here, with Kintu &amp; Nambi — a Baganda legend about the first man and woman, carried from an oral tradition into a world of curious minds.</p>
...
function StorySection({ onRead }: { onRead: () => void }) {
  return (
    <section id="story" className="overflow-hidden bg-[hsl(var(--primary))] px-5 py-24 text-[hsl(var(--background))] md:px-10 md:py-32">
      <div className="mx-auto max-w-[1180px]">
        <div className="flex items-start justify-between gap-5">
          <div><p className="font-mono-label text-[10px] uppercase tracking-[.2em] text-[hsl(var(--background)/.68)]">02 / The legend</p><h2 className="mt-7 max-w-3xl font-display text-6xl leading-[.85] md:text-9xl">The first<br /><span className="italic">two.</span></h2></div>
          <span className="font-mono-label mt-1 text-[10px] text-[hsl(var(--background)/.5)]">8 min read</span>
        </div>
        <div className="mt-20 grid gap-12 md:grid-cols-[.8fr_1.2fr] md:items-end">
          ...
          <div className="md:pb-2">
            ...
            <button onClick={onRead} className="story-link mt-10 flex items-center gap-3 border-b border-[hsl(var(--accent))] pb-2 font-mono-label text-[10px] uppercase tracking-[.16em] text-[hsl(var(--accent))]">Open the reading room <ArrowUpRight size={15} /></button>
          </div>
        </div>
      </div>
    </section>
...
function Home() {
  ...
  return <div className="texture min-h-[100dvh]"><Header onOpenMenu={() => setMenuOpen(true)} /><MobileMenu open={menuOpen} close={() => setMenuOpen(false)} /><main><Hero onRead={() => setReading(true)} /><Introduction /><StorySection onRead={() => setReading(true)} /><ListenSection /><WorldSection /><HouseSection /></main><Footer />{reading && <ReadingRoom close={() => setReading(false)} />}</div>;
}
